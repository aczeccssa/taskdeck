...
const detailedDescriptions:Record<string,string>={
  ...
  "releases:Versioned docs":"Latest documentation is convenient for new users, but reproducible deployments need a versioned path. The Pages build stores stable tags below `/versions/<tag>/`. While `Cargo.toml` still matches the current release, documentation fixes on `master` also rebuild that release path; older versions remain built from their original tags. The version index records each release URL, checksum URL, and documentation path.",
