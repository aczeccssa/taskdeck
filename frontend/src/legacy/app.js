...
function setSidebar(collapsed) {
    ...
    if (brand) {
        ...
        brand.setAttribute("aria-label", collapsed ? "Expand navigation" : "Collapse navigation");
        brand.title = collapsed ? "Expand navigation" : "Collapse navigation";
    }
    localStorage.setItem("taskdeck-sidebar-collapsed", String(collapsed));
}


function setAlertsEditorCollapsed(collapsed) {
    const layout = $(".alerts-layout");
    const toggle = $("#toggle-alert-rules");
    if (!layout || !toggle) return;
    layout.classList.toggle("editor-collapsed", collapsed);
    toggle.setAttribute("aria-expanded", String(!collapsed));
    toggle.setAttribute("aria-label", collapsed ? "Expand alert rules" : "Collapse alert rules");
    toggle.title = collapsed ? "Expand alert rules" : "Collapse alert rules";
    toggle.textContent = collapsed ? "+" : "×";
    localStorage.setItem("taskdeck-alert-rules-collapsed", String(collapsed));
}
...
function setTheme() {
    ...
    themeButton?.setAttribute("aria-label", `Color theme: ${next}`);
...
function applySavedPreferences() {
    ...
    setAlertsEditorCollapsed(localStorage.getItem("taskdeck-alert-rules-collapsed") !== "false");
...
const routePaths = {
    tasks: "/", dashboard: "/dashboard", workflows: "/workflows", boards: "/boards",
    alerts: "/alerts", calls: "/calls", audit: "/audit", docs: "/docs", settings: "/settings",
};

function viewForPath(pathname) {
    const path = pathname.replace(/\/+$/, "") || "/";
    return Object.entries(routePaths).find(([, route]) => route === path)?.[0] || "tasks";
}

function setView(view, syncRoute = true) {
    if (!(view in routePaths)) view = "tasks";
    if (syncRoute && window.location.pathname !== routePaths[view]) {
        window.dispatchEvent(new CustomEvent("taskdeck:route-request", {detail: view}));
    }
    state.view = view;
    ...
    sessionSelect.hidden = false;
    const sessionControl = sessionSelect.closest(".select-control");
    if (sessionControl) sessionControl.hidden = view !== "tasks";
    $("#nodes").hidden = false;
...
async function loadWorkspaces() {
    ...
    try {
        ...
        if (!$("#sessions").options.length || [...$("#sessions").options].some((option) => !sessions.includes(option.value))) {
            ...
            [...$("#sessions").options].forEach((option) => {
                option.textContent = sessionOptionLabel(option.value);
            });
        }
        renderAliasForm();
        setConnection(true);
...
function renderWorkflows() {
    ...
    groups.innerHTML = state.workflowGroups.length
        ...
        : '<div class="empty-state compact"><div><h1>No execution workflows</h1><p>Connect tasks across nodes, define their order, and run the flow.</p></div></div>';
...
function renderWorkflowCard(group) {
    const members = group.members?.length
        ? group.members.map((member, index) => `<li class="workflow-step ${member.available ? "" : "unavailable"}"><span class="workflow-step-number">${index + 1}</span><button class="workflow-member" type="button" data-workflow-open data-node="${escapeAttr(member.node_id)}" data-session="${escapeAttr(member.session)}"><span class="status-pill ${member.available ? "" : "error"}">${escapeHtml(member.available ? "ready" : member.skip_reason || "skipped")}</span><strong>${escapeHtml(member.task || "Untitled task")}</strong><span>${escapeHtml(member.node_name || member.node_id)} · ${escapeHtml(member.workspace_display_name || member.session)}</span></button>${index < group.members.length - 1 ? '<span class="workflow-step-arrow" aria-hidden="true">↓</span>' : ""}</li>`).join("")
        : '<li class="muted">No execution steps yet.</li>';
    const graphEdges = group.graph?.edges?.length || Math.max(0, (group.members?.length || 0) - 1);
    return `<article class="workflow-card" data-workflow-id="${escapeAttr(group.id)}">
    <header><div><h2>${escapeHtml(group.name)}</h2><p>${group.members?.length || 0} execution step${(group.members?.length || 0) === 1 ? "" : "s"} · ${graphEdges} connection${graphEdges === 1 ? "" : "s"}</p></div><div class="workflow-card-actions"><button class="button compact" type="button" data-workflow-edit="${escapeAttr(group.id)}">Edit graph</button><button class="button compact danger" type="button" data-workflow-delete="${escapeAttr(group.id)}">Delete</button></div></header>
    <ol class="workflow-steps">${members}</ol>
...
function beginWorkflowEditor(group = null) {
    ...
    $("#workflows-view")?.classList.add("editor-open");
...
function cancelWorkflowEditor() {
    ...
    $("#workflows-view")?.classList.remove("editor-open");
...
async function saveWorkflow() {
    ...
    const body = {
        name, members: state.workflowDraftMembers, graph: {
            positions: orchestratorGraph().positions.slice(0, state.workflowDraftMembers.length).map((position) => ({
                x: position.x,
                y: position.y
            })),
...
function renderBoardOverview() {
    ...
    if (nodes) {
        nodes.innerHTML = state.nodes.length
            ? state.nodes.map((node) => `<span class="board-scope-node ${node.online ? "" : "offline"}"><i aria-hidden="true"></i><strong>${escapeHtml(node.is_self ? `This device · ${node.name}` : node.name)}</strong><small>${escapeHtml(node.role || "node")} · ${node.sessions?.length || 0} workspace${(node.sessions?.length || 0) === 1 ? "" : "s"}</small></span>`).join("")
            : '<span class="muted">No nodes known.</span>';
    }
    const workspaces = $("#board-workspaces");
    if (workspaces) {
        workspaces.innerHTML = state.boardTargets.length
            ? state.boardTargets.map((target) => `<button class="board-source" type="button" data-board-open data-node="${escapeAttr(target.node_id)}" data-session="${escapeAttr(target.session)}"><strong>${escapeHtml(target.workspace_display_name)}</strong><span>${escapeHtml(target.node_name)} · ${target.tasks?.length || 0} task${(target.tasks?.length || 0) === 1 ? "" : "s"}</span></button>`).join("")
            : '<span class="muted">No workspaces registered.</span>';
    }
}
...
function renderBoardList() {
    ...
    list.innerHTML = state.boards.length
        ...
        <div class="board-card-grid">${board.cards?.length ? board.cards.map((card) => boardCardMarkup({
            ...card,
            key: boardCardKey(card)
        })).join("") : '<div class="muted">No cards yet. Edit the board to add task cards.</div>'}</div>
      </article>`).join("")
...
async function loadBoardCardLogs(card) {
    try {
        ...
        if (!response.ok) throw new Error(response.message);
        state.boardCardData[card.key] = {
            ...(state.boardCardData[card.key] || {}),
            logs: {lines: response.data?.lines || []}
        };
    } catch (error) {
        state.boardCardData[card.key] = {
            ...(state.boardCardData[card.key] || {}),
            logs: {lines: [], error: error.message || "Logs unavailable"}
        };
    }
}
...
function beginBoardEditor(board = null) {
    ...
    $("#boards-view")?.classList.add("editor-open");
...
function cancelBoardEditor() {
    ...
    $("#boards-view")?.classList.remove("editor-open");
...
async function loadNodeSettings() {
    ...
    try {
        ...
        renderNodeSettings();
        subtitle.textContent = `${state.nodeSettings.settings.name || node} · ${state.nodeSettings.settings.node_id || node}`;
    } catch (error) {
...
function updateMeta() {
    const meta = $("#meta");
    if (state.view === "calls") {
        const summary = `${state.callPage.total || 0} retained call${state.callPage.total === 1 ? "" : "s"}`;
        meta.textContent = summary;
    } else if (state.view === "audit") {
        const summary = `${state.auditPage.total || 0} audit record${state.auditPage.total === 1 ? "" : "s"}`;
        meta.textContent = summary;
    } else if (state.view === "runs") {
        meta.textContent = `${state.runPage.total || 0} run${(state.runPage.total || 0) === 1 ? "" : "s"}`;
    } else if (state.view === "dashboard") {
        meta.textContent = "Live node and task resources.";
    } else if (state.view === "alerts") {
        meta.textContent = "Notify on task transitions; optionally POST the event to a webhook.";
    } else if (state.view === "docs") {
        meta.textContent = "Use the local endpoint to inspect and control registered sessions.";
    } else if (state.view === "settings") {
        meta.textContent = "Configure nodes, workspace aliases, and daemon startup.";
    } else if (state.view === "workflows") {
        meta.textContent = "Build an execution graph across nodes, connect task steps, then run the flow.";
    } else if (state.view === "boards") {
        meta.textContent = "Pin tasks from any node and workspace, then choose what each card shows.";
...
function renderTaskHeader(task) {
    ...
    $("#task-header").innerHTML = `<div class="task-heading">
    <div class="eyebrow">${online ? "Selected task" : "Offline snapshot"}</div>
    <h1>${escapeHtml(task.label)}</h1>
    <div class="task-detail"><span class="status ${escapeAttr(status)}">${escapeHtml(status)}</span>${task.pid ? `<span>PID ${task.pid}</span>` : ""}${technologyLabel ? `<span class="technology-chip" title="${escapeAttr((technology.evidence || []).join(" · "))}">${escapeHtml(technologyLabel)}</span>` : ""}${endpointMarkup}<span
            class="cwd">${escapeHtml(task.cwd)}</span></div>
</div>
<div class="view-modes" aria-label="Workspace layout">
    <button type="button" data-mode="log">Logs</button>
...
<div class="actions">
    ...
    <button class="icon-button" type="button" data-config aria-label="Edit configuration" title="Edit configuration"
            ${online ? "" : "disabled"}>${icons.settings}
    </button>
</div>`;
    applyWorkspaceMode();
...
function mcpTarget(input, targetNode) {
    ...
    if (input?.session) return {
        primary: input.session,
        secondary: `${node} · ${input.tail ? `Last ${input.tail} lines` : "Session"}`
    };
...
function auditTarget(record) {
    ...
    return {
        primary: record.operation || record.request_kind || "Request",
        secondary: record.correlation_id || "No target"
    };
}
...
async function loadConfig(session, confirmDiscard) {
    ...
    try {
        ...
        state.configWorkspaceEnvRows = Object.entries(response.data.workspace_env || {}).map(([key, value]) => ({
            key,
            value
        }));
...
async function loadRuns() {
    ...
    if (runs.ok) {
        state.runPage = {
            page: runs.data.page,
            page_size: runs.data.page_size,
            total: runs.data.total,
            total_pages: runs.data.total_pages
        };
        state.runs = runs.data.items || [];
    } else throw new Error(runs.message);
    if (eventResponse.ok) {
        state.eventPage = eventResponse.data;
        state.events = eventResponse.data.items || [];
    } else throw new Error(eventResponse.message);
    renderRuns();
    updateMeta();
}
...
async function validateWorkspaceEnv() {
    const values = {};
    state.configWorkspaceEnvRows.forEach((row) => {
        const key = row.key.trim();
    ...
    try {
        ...
        const response = await requestJson(`/api/sessions/${encodeURIComponent(session)}/config?${addNodeQuery()}`, {
            ...
            body: JSON.stringify({
                ...
                workspace_env: Object.fromEntries(validateWorkspaceEnv()),
                tasks
            }),
        });
        if (!response.ok) {
...
function customizeSelect(select) {
    ...
    menu.id = `${select.id || "select"}-options`;
    ...
    trigger.addEventListener("click", () => {
        ...
        document.querySelectorAll(".select-control.is-open").forEach((item) => {
    ...
    trigger.addEventListener("keydown", (event) => {
        if (["Enter", " ", "ArrowDown", "ArrowUp"].includes(event.key)) {
            event.preventDefault();
            trigger.click();
            menu.querySelector(".custom-select-option[aria-selected=true]")?.focus();
        }
...
function enhanceCustomSelects(root = document) {
    const selects = root instanceof HTMLSelectElement ? [root] : $$("select", root);
    selects.forEach(customizeSelect);
}

document.addEventListener("click", (event) => {
    if (event.target instanceof Element && event.target.closest(".select-control")) return;
    document.querySelectorAll(".select-control.is-open").forEach((wrapper) => {
        wrapper.classList.remove("is-open");
...
function bindEvents() {
    $("#brand-toggle").addEventListener("click", () => setSidebar(!$("#app-shell").classList.contains("sidebar-collapsed")));
    ...
    $("#calls-search").addEventListener("input", (event) => {
        state.callFilters.q = event.target.value.trim();
        scheduleCallsReload();
    });
    $("#calls-operation").addEventListener("change", (event) => {
        state.callFilters.operation = event.target.value;
        scheduleCallsReload();
    });
    $("#calls-status").addEventListener("change", (event) => {
        state.callFilters.status = event.target.value;
        scheduleCallsReload();
    });
    $("#calls-session").addEventListener("input", (event) => {
        state.callFilters.session = event.target.value.trim();
        scheduleCallsReload();
    });
    $("#calls-task").addEventListener("input", (event) => {
        state.callFilters.task = event.target.value.trim();
        scheduleCallsReload();
    });
    $("#calls-page-size").addEventListener("change", (event) => {
        state.callFilters.pageSize = Number(event.target.value);
        scheduleCallsReload();
    });
    $("#clear-call-filters").addEventListener("click", () => {
        state.callFilters = {
            ...
            task: "",
            page: 1,
            pageSize: Number($("#calls-page-size").value) || 20
        };
        $("#calls-search").value = "";
        $("#calls-operation").value = "";
        $("#calls-status").value = "all";
        $("#calls-session").value = "";
        $("#calls-task").value = "";
        loadMcpCalls();
    });
    $("#calls-prev").addEventListener("click", () => {
        if (state.callPage.has_previous) {
            state.callFilters.page -= 1;
            loadMcpCalls();
        }
    });
    $("#calls-next").addEventListener("click", () => {
        if (state.callPage.has_next) {
            state.callFilters.page += 1;
            loadMcpCalls();
        }
    });
    ...
    $("#audit-source").addEventListener("change", (event) => {
        state.auditFilters.source = event.target.value;
        scheduleAuditReload();
    });
    $("#audit-status").addEventListener("change", (event) => {
        state.auditFilters.status = event.target.value;
        scheduleAuditReload();
    });
    $("#audit-node").addEventListener("input", (event) => {
        state.auditFilters.node = event.target.value.trim();
        scheduleAuditReload();
    });
    $("#audit-session").addEventListener("input", (event) => {
        state.auditFilters.session = event.target.value.trim();
        scheduleAuditReload();
    });
    $("#audit-task").addEventListener("input", (event) => {
        state.auditFilters.task = event.target.value.trim();
        scheduleAuditReload();
    });
    $("#audit-operation").addEventListener("input", (event) => {
        state.auditFilters.operation = event.target.value.trim();
        scheduleAuditReload();
    });
    $("#audit-page-size").addEventListener("change", (event) => {
        state.auditFilters.pageSize = Number(event.target.value);
        scheduleAuditReload();
    });
    $("#clear-audit-filters").addEventListener("click", () => {
        state.auditFilters = {
            ...
            operation: "",
            page: 1,
            pageSize: Number($("#audit-page-size").value) || 20
        };
        $("#audit-search").value = "";
        $("#audit-source").value = "all";
        $("#audit-status").value = "all";
        $("#audit-node").value = "";
        $("#audit-session").value = "";
        $("#audit-task").value = "";
    ...
    $("#audit-prev").addEventListener("click", () => {
        if (state.auditPage.has_previous) {
            state.auditFilters.page -= 1;
            loadAudit();
        }
    });
    $("#audit-next").addEventListener("click", () => {
        if (state.auditPage.has_next) {
            state.auditFilters.page += 1;
            loadAudit();
        }
    });
    ...
    $("#call-dialog").addEventListener("click", (event) => {
        if (event.target === $("#call-dialog")) closeCallDetails();
    });
    $("#call-dialog").addEventListener("close", () => {
        state.callDetailRequest += 1;
        state.callDetailId = null;
    });
    $("#audit-dialog").addEventListener("click", (event) => {
        if (event.target === $("#audit-dialog")) closeAuditDetails();
    });
    ...
    $("#config-dialog").addEventListener("click", (event) => {
        if (event.target === $("#config-dialog")) requestCloseConfig();
    });
    ...
    $("#close-workflow-editor")?.addEventListener("click", cancelWorkflowEditor);
    ...
    $("#close-board-editor")?.addEventListener("click", cancelBoardEditor);
    ...
    $("#runs-next")?.addEventListener("click", () => {
        if (state.runPage.has_next ?? (state.runFilters.page < state.runPage.total_pages)) {
            state.runFilters.page += 1;
            loadRuns().catch(() => {
            });
        }
    ...
    document.addEventListener("fullscreenchange", () => {
        if (!document.fullscreenElement) $("#log-panel")?.classList.remove("fallback-fullscreen");
        updateFullscreenButton();
    });
    ...
    $("#alerts-bell")?.addEventListener("click", toggleNotificationPopover);
    $("#notification-popover-all")?.addEventListener("click", () => { closeNotificationPopover(); setView("alerts"); });
    $("#notification-popover-list")?.addEventListener("click", (event) => {
        const button = event.target.closest("[data-notification-popover-read]");
        if (button) markNotificationsRead(button.dataset.notificationPopoverRead);
    });
    document.addEventListener("click", (event) => {
        if (!event.target.closest(".notification-menu-wrap")) closeNotificationPopover();
    });
    $("#toggle-alert-rules")?.addEventListener("click", () => setAlertsEditorCollapsed(!$(".alerts-layout")?.classList.contains("editor-collapsed")));
    $("#open-alert-rules")?.addEventListener("click", () => setAlertsEditorCollapsed(false));
    ...
    $("#open-scaling-settings")?.addEventListener("click", () => $("#scaling-dialog")?.showModal());
    $("#close-scaling-settings")?.addEventListener("click", () => $("#scaling-dialog")?.close());
...
function handleConfigInput(event) {
    const target = event.target;
    if (target.dataset.workspaceKey != null) {
        state.configWorkspaceEnvRows[Number(target.dataset.workspaceKey)].key = target.value;
    } else if (target.dataset.workspaceValue != null) {
        state.configWorkspaceEnvRows[Number(target.dataset.workspaceValue)].value = target.value;
    } else {
        const task = state.configTasks[state.configTaskIndex];
        if (!task) return;
...
function handleConfigButton(event) {
    ...
    const button = event.target.closest("button");
    if (!button) return;
    if (button.dataset.addWorkspace != null) {
        state.configWorkspaceEnvRows.push({key: "", value: ""});
        renderConfigWorkspace();
    } else if (button.dataset.removeWorkspace != null) {
        state.configWorkspaceEnvRows.splice(Number(button.dataset.removeWorkspace), 1);
        renderConfigWorkspace();
    } else if (!task) return;
    if (button.dataset.addArg != null) {
        task.args.push("");
        renderConfigForm();
    }
    if (button.dataset.removeArg != null) {
        task.args.splice(Number(button.dataset.removeArg), 1);
        renderConfigForm();
    }
    if (button.dataset.addEnv != null) {
        task.envRows.push({key: "", value: ""});
        renderConfigForm();
    }
    if (button.dataset.removeEnv != null) {
        task.envRows.splice(Number(button.dataset.removeEnv), 1);
        renderConfigForm();
    }
...
const I18N = {
    en: {
        ...
        "action.runInOrder": "Run in order",
        "action.history": "History",
        "action.addDependency": "Add dependency",
        "action.addQuota": "Add quota",
        ...
        "action.applyTemplate": "Create board",
        "action.exportTemplate": "Export",
        "action.importTemplate": "Import",
        "action.deleteTemplate": "Delete",
        "action.addCard": "Add card",
        "action.saveBoard": "Save board",
        "dashboard.title": "Dashboard",
        "dashboard.subtitle": "Live node and task resources.",
        ...
        "workflows.orchestratorHint": "Build an execution graph across nodes, connect task steps, then run the flow in order.",
        ...
        "alerts.title": "Alerts",
        "alerts.subtitle": "0 unread notifications",
        ...
        "event.taskExited": "Task exited",
        "event.taskFailed": "Task failed",
        "event.taskStopped": "Task stopped",
        "field.name": "Name",
        "field.policyName": "Policy name",
        "field.watchNode": "Watch node",
        "field.watchWorkspace": "Watch workspace",
        "field.watchTask": "Watch task",
        "field.metric": "Metric",
        "field.scaleOutAbove": "Scale out above",
        "field.scaleInBelow": "Scale in below",
        "field.replicaNode": "Replica node",
        "field.replicaWorkspace": "Replica workspace",
        "field.replicaTask": "Replica task",
        "field.cooldown": "Cooldown (seconds)",
        "field.taskNode": "Task node",
        "field.taskWorkspace": "Task workspace",
        "field.task": "Task",
        "field.dependsNode": "Depends on node",
        "field.dependsWorkspace": "Depends on workspace",
        "field.dependsTask": "Depends on task",
        "field.ruleName": "Rule name",
        "field.workspaceScope": "Workspace (optional)",
        "field.taskScope": "Task (optional)",
        "field.webhook": "Webhook URL (optional)",
        "field.enabled": "Enabled",
        "field.templateName": "Template name",
        "field.templateSource": "From board",
        "field.maxRunning": "Max running tasks",
        "field.tokenName": "Token name",
    ...
    zh: {
        ...
        "workflows.orchestratorHint": "在多个节点上编排任务步骤，连接执行关系，然后按顺序运行。",
...
function applyI18n() {
    ...
    $("#lang").textContent = state.lang === "en" ? "EN" : "中";
...
function renderDashboard() {
    ...
    nodes.innerHTML = (data.nodes || []).map((node) => {
      ...
      <header><div><strong>${escapeHtml(node.node_name || node.node_id)}</strong><small>${escapeHtml(node.node_id)}</small></div><span class="status-pill ${node.online ? "" : "error"}">${node.online ? "online" : "offline"}</span></header>
      <div class="dashboard-metrics">
        <div class="gauge"><span>${t("dashboard.cpu")}</span><strong>${cpu ?? "–"}%</strong><div class="gauge-bar"><i style="width:${cpu ?? 0}%"></i></div></div>
        <div class="gauge"><span>${t("dashboard.memory")}</span><strong>${memLabel}</strong><div class="gauge-bar memory"><i style="width:${memPercent}%"></i></div></div>
      </div>
      <div class="dashboard-node-stats"><span><b>${current?.running_tasks ?? 0}</b> ${t("dashboard.runningTasks")}</span><span><b>${node.session_count}</b> ${t("dashboard.sessions")}</span>${statusChips(node.task_status_counts)}</div>
...
async function submitScalingPolicy(event) {
    ...
    try {
        ...
        const response = await requestJson(url, {
            method: state.scalingEditingId ? "PUT" : "POST",
            headers: {"content-type": "application/json"},
            body: JSON.stringify(body)
        });
        if (!response.ok) throw new Error(response.message);
...
function toggleNotificationPopover() {
    ...
    popover.hidden = !state.notificationPopoverOpen;
    bell.setAttribute("aria-expanded", String(state.notificationPopoverOpen));
...
function renderNotificationPopover() {
    const list = $("#notification-popover-list");
    const count = $("#notification-popover-count");
    if (!list) return;
    const recent = [...state.notifications].sort((a, b) => Number(b.created_at_ms || 0) - Number(a.created_at_ms || 0)).slice(0, 5);
    if (count) count.textContent = state.unreadCount ? `${state.unreadCount} unread` : "";
    list.innerHTML = recent.length ? recent.map((item) => `<article class="notification-popover-item ${item.read ? "read" : "unread"}">
        <div><span class="status-dot ${severityClass(item.severity)}"></span><strong>${escapeHtml(item.title)}</strong></div>
        <p>${escapeHtml(item.message)}</p>
        <small>${escapeHtml(formatTimestamp(item.created_at_ms))}${item.session ? ` · ${escapeHtml(item.session)}${item.task ? ` · ${escapeHtml(item.task)}` : ""}` : ""}</small>
        ${item.read ? "" : `<button class="notification-popover-read" type="button" data-notification-popover-read="${escapeAttr(item.id)}">Mark read</button>`}
    </article>`).join("") : `<div class="notification-popover-empty">No notifications yet.</div>`;
}
...
function renderAlerts() {
    ...
    const meta = $("#meta");
    if (state.view === "alerts" && meta) meta.textContent = "Notify on task transitions; optionally POST the event to a webhook.";
    if (list) {
...
function beginRuleEditor(rule = null) {
    setAlertsEditorCollapsed(false);
...
async function submitRule(event) {
    ...
    try {
        ...
        const response = await requestJson(url, {
            method: state.ruleEditingId ? "PUT" : "POST",
            headers: {"content-type": "application/json"},
            body: JSON.stringify(body)
        });
        if (!response.ok) throw new Error(response.message);
...
async function markNotificationsRead(id) {
    try {
        ...
        const response = await requestJson("/api/notifications/read", {
            method: "POST",
            headers: {"content-type": "application/json"},
            body: JSON.stringify(body)
        });
...
async function saveOrchestratorLayout() {
    ...
    if (!group) return;
    const graph = {
        positions: orchestratorGraph().positions.slice(0, state.workflowDraftMembers.length).map((p) => ({
            x: p.x,
            y: p.y
        })),
        edges: orchestratorGraph().edges.map((edge) => ({from: edge.from, to: edge.to})),
...
async function applyBoardTemplate() {
    ...
    try {
        const response = await requestJson(`/api/board-templates/${encodeURIComponent(state.selectedTemplateId)}/apply`, {
            method: "POST",
            headers: {"content-type": "application/json"},
...
async function importBoardTemplate(file) {
    try {
        ...
        const response = await requestJson("/api/board-templates/import", {
            method: "POST",
            headers: {"content-type": "application/json"},
...
window.addEventListener("popstate", () => setView(viewForPath(window.location.pathname), false));
window.addEventListener("taskdeck:navigate", (event) => {
    const view = event instanceof CustomEvent ? event.detail : "tasks";
    setView(view, false);
});
...
const selectObserver = new MutationObserver((records) => {
    records.forEach((record) => {
        if (record.target instanceof HTMLSelectElement) customizeSelect(record.target);
        record.addedNodes.forEach((node) => {
            if (!(node instanceof Element)) return;
            const select = node instanceof HTMLSelectElement ? node : node.closest("select");
            if (select) customizeSelect(select);
            else enhanceCustomSelects(node);
        });
    });
});
selectObserver.observe(document.body, {childList: true, subtree: true});
updateEndpoint();
setView(viewForPath(window.location.pathname), false);
loadNodes();
