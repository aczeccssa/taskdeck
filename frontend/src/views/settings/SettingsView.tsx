...
export function SettingsView(): React.JSX.Element {
    ...
    const [selfSettings] = useRequest<NodeSettingsView>("/api/nodes/self/settings", {} as NodeSettingsView, (value) => (value as {settings?: NodeSettingsView}).settings ?? value as NodeSettingsView);
    ...
    const saveNode = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); const payload = {...Object.fromEntries(form), allow_remote_bind: form.get("allow_remote_bind") === "on"}; setMessage("Saving…"); const result = await post(`/api/nodes/${encodeURIComponent(nodeId)}/settings`, payload); setMessage(result.ok ? "Node settings saved." : result.message); if (result.ok) { reload(); void selection.refresh(); } };
    ...
    const removeWorkspace = async (session: string): Promise<void> => {
        if (!window.confirm(`Remove workspace '${session}'? Its Taskdeck registration and live task state will be removed from this node.`)) return;
        const result = await api.request(`/api/workspaces/${encodeURIComponent(session)}?${new URLSearchParams({node: nodeId})}`, decode, {method: "DELETE"});
        setMessage(result.ok ? `Workspace '${session}' removed.` : result.message);
    ...
    const createToken = async (event: FormEvent<HTMLFormElement>) => {
        ...
        const result = await post("/api/tokens", {name: form.get("name")});
        const data = result.data as {secret?: unknown} | undefined;
        setCreatedSecret(result.ok && typeof data?.secret === "string" ? data.secret : null);
        setMessage(result.ok ? "Token created. Copy it now; it cannot be shown again." : result.message);
        if (result.ok) event.currentTarget.reset();
        tokenReload();
    };
    const copyCreatedToken = async (): Promise<void> => {
        if (!createdSecret) return;
        try {
            await navigator.clipboard.writeText(createdSecret);
            setMessage("Token copied.");
        } catch {
            setMessage("Copy is unavailable in this browser. Select the token text and copy it manually.");
        }
    };
    const revokeToken = async (token: {id: string; name: string}): Promise<void> => {
        if (!window.confirm(`Revoke token '${token.name}'? Clients using it will lose access immediately.`)) return;
        setRevokingToken(token.id);
        const result = await api.request(`/api/tokens/${encodeURIComponent(token.id)}`, decode, {method: "DELETE"});
        setRevokingToken(null);
        setMessage(result.ok ? `Token '${token.name}' revoked.` : result.message);
        if (result.ok) tokenReload();
    };
    const isPureMaster = selfSettings.role === "leader" && selfSettings.leader_mode === "pure_master";
        ...
        <div className="settings-shell">
            ...
            <div className="settings-content" key={`${section}-${nodeId}`}>
                {section === "nodes" && <>
                    ...
                    <section className="settings-panel"><header><div><h2>Node configuration</h2><p>{busy ? "Loading…" : error || "The display name is editable; the stable ID is not."}</p></div><code>{nodeId}</code></header>{error ? <p className="form-error">{error}</p> : <form className="settings-form" onSubmit={saveNode}><label className="field"><span>Display name</span><input name="name" defaultValue={settings.name ?? ""} required /></label><details><summary>Advanced node configuration</summary><div className="field-grid"><label className="field"><span>Role</span><select name="role" defaultValue={settings.role ?? "worker"}><option value="worker">Worker</option><option value="leader">Leader</option></select></label><label className="field"><span>Leader URL</span><input name="leader_url" defaultValue={settings.leader_url ?? ""}/></label><label className="field"><span>Bind host</span><input name="bind_host" defaultValue={settings.bind_host ?? ""}/></label><label className="checkbox-field"><input name="allow_remote_bind" type="checkbox" defaultChecked={settings.allow_remote_bind === true}/><span>Allow remote bind (explicit network exposure)</span></label><label className="field"><span>Web port</span><input name="web_port" type="number" defaultValue={settings.web_port ?? ""}/></label></div></details><p className="settings-message" role="status">{message}</p><button className="button primary" type="submit">Save node</button></form>}</section>
                </>}
                {section === "workspaces" && <section className="settings-panel"><header><div><h2>Workspaces</h2><p>Rename a workspace or remove a registration that is no longer valid.</p></div></header>{selection.workspaces.length ? selection.workspaces.map((workspace) => <WorkspaceAlias key={workspace.session} session={workspace.session} alias={workspace.alias ?? ""} onSave={renameWorkspace} onRemove={removeWorkspace}/>) : <p className="muted">No workspaces on this node.</p>}<details><summary>Advanced resource quotas</summary>{quotaError ? <p className="form-error">{quotaError}</p> : quotas.quotas.map((quota) => <div className="quota-row" key={quota.id}><strong>{quota.session || "Node default"}</strong><span>Maximum {quota.max_running_tasks} running</span><button className="button compact danger" onClick={() => void post(`/api/quotas/${encodeURIComponent(quota.id)}`, undefined, "DELETE").then(quotaReload)}>Delete</button></div>)}<form className="settings-form" onSubmit={addQuota}><div className="field-grid"><label className="field"><span>Workspace</span><select name="session"><option value="">Node default</option>{selection.sessions.map((session) => <option key={session}>{session}</option>)}</select></label><label className="field"><span>Maximum running tasks</span><input name="max_running_tasks" type="number" min="1" defaultValue="2"/></label></div><button className="button" type="submit">Add quota</button></form></details><p className="settings-message" role="status">{message}</p></section>}
                {section === "automation" && <ScalingSettings/>}
                {section === "access" && <section className="settings-panel"><header><div><h2>API access</h2><p>Create integration credentials and revoke them when no longer needed.</p></div><button className="button" onClick={tokenReload}>Refresh</button></header>{createdSecret && <div className="token-secret"><div><strong>Copy this token now</strong><p>It is shown once and cannot be recovered later.</p><code>{createdSecret}</code></div><div className="settings-actions"><button className="button" onClick={() => void copyCreatedToken()}>Copy</button><button className="button" onClick={() => setCreatedSecret(null)}>I've saved it</button></div></div>}{tokenError ? <p className="form-error">{tokenError}</p> : tokens.tokens.length ? <div className="token-list">{tokens.tokens.map((token) => <div className={`token-row${token.revoked ? " revoked" : ""}`} key={token.id}><div><strong>{token.name}</strong><code>{token.token_prefix}…</code></div><small>Created {formatTimestamp(token.created_at_ms)}{token.last_used_at_ms ? ` · Used ${formatTimestamp(token.last_used_at_ms)}` : " · Never used"}</small>{token.revoked ? <span className="muted">Revoked</span> : <button className="button compact danger" disabled={revokingToken === token.id} onClick={() => void revokeToken(token)}>{revokingToken === token.id ? "Revoking…" : "Revoke"}</button>}</div>)}</div> : <p className="muted">No API tokens yet.</p>}<form className="settings-form token-create-form" onSubmit={createToken}><label className="field"><span>Token name</span><input name="name" required placeholder="CI agent"/></label><button className="button primary">Create token</button></form><p className="settings-message" role="status">{message}</p></section>}
                {section === "runtime" && <section className="settings-panel"><header><div><h2>{isPureMaster ? "Leader runtime" : "Local runtime"}</h2><p>{isPureMaster ? "This pure-master leader is managed by Docker Compose." : "Native service controls apply only to the device running this UI."}</p></div>{node?.is_self && !isPureMaster && <button className="button" onClick={serviceReload}>Refresh</button>}</header>{isPureMaster ? <div className="empty-state compact"><div><h3>Managed by Compose</h3><p>Start, stop, and restart the leader through Docker Compose. Worker runtime controls remain available from each worker's own Taskdeck UI.</p></div></div> : node?.is_self ? <><p className="muted">{serviceError || service.status || (service.running ? "Running" : "Stopped")}</p><div className="settings-actions">{["install", "uninstall", "start", "stop"].map((action) => <button className="button" key={action} onClick={async () => { const result = await post("/api/nodes/self/service", {action, scope: "user"}); setMessage(result.ok ? "Service operation completed." : result.message); serviceReload(); }}>{action[0].toUpperCase() + action.slice(1)}</button>)}</div></> : <div className="empty-state compact"><div><h3>Remote node selected</h3><p>Runtime controls are intentionally unavailable because they would operate on this local machine, not {node?.name ?? nodeId}.</p></div></div>}<p className="settings-message" role="status">{message}</p></section>}
            </div>
        </div>
    </section>;
}
...
function WorkspaceAlias({session, alias, onSave, onRemove}: {session: string; alias: string; onSave: (session: string, alias: string) => Promise<void>; onRemove: (session: string) => Promise<void>}): React.JSX.Element {
    const [value, setValue] = useState(alias);
    return <div className="workspace-alias-row"><div><strong>{alias || session}</strong><code>{session}</code></div><input aria-label={`Display name for ${session}`} value={value} onChange={(event) => setValue(event.target.value)} placeholder="Display name"/><button className="button compact" onClick={() => void onSave(session, value)}>Save</button><button className="button compact danger" onClick={() => void onRemove(session)}>Remove</button></div>;
}
